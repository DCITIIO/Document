<center>Connecting Smart Cities to Blockchain And Creating A Crypto Based Economy For Free Communities</center>
...
BitCiti Concept Demitisfied BitCiti Belief of the Future
2
________________
...
While DCiti is the world’s first all-in-one blockchain protocol bridging the gap between smart technologies such as Internet of Things, Sensor, wireless network Tech, and blockchain to provide decentralized services to existing and new smart cities project all over the world, BitCiti
6
________________
will be our first practical test of a decentralized smart city and free community built with 100% decentralized system, operating a crypto based economy and using DCiti blockchain.
...
Developers can build Dapps to be integrated with Internet of Things to make the collection of accurate data easily possible and assembled on a decentralized distributed network of
10
________________
computers. The actual purpose of DCiti system is to be the decentralized data grid distributed across networks of computers that makes all other Dapps and smart contract system to function maximally.
...
Each network participant vote is proportional to their stake, users can also delegate their voting power to another user that can vote on their behalf. A block released by one producer is validated by the next and the next and so forth; if not validated, it is not built upon. Similar longest-chain mechanics to Bitcoin are followed, and in short order, the producers converge on a longest chain.
...
DPOS avoids the tax of mining, releasing that substantial value back to stakeholders. Value from block rewards would be initially captured entirely by the producers. However, because they are elected by the community, they are incentivized to share the rewards by a scheme that producers agree on amongst themselves, and promote to the community
16
________________
DCiti Use Cases
...
18
________________
2 BitCiti Smart City Architecture
...
19
________________
Land purchases, leasing and every day transactions will all take place using these cryptocurrency.
...
contracts and speculation, but something we use in our everyday lives. 2. We envision a future where sustainable wood technology like CLT completely replaces conventional building materials because of its clear advantages over both steel and concrete. 3. We envision a type of city we would love to live in, so we believe that with
...
Our number one goal is to build BitCiti as a human friendly, technology driven - Smart living environment and voluntary community. The end target is a community built smart city, attracting cryptocurrency lovers, technology enthusiasts, adventurous tourists. And becoming an
20
________________
accelerator hub for the next generation of blockchain businesses and decentralized autonomous communities.
...
Redefining smart city by combining different concepts to produce a great city. Though BitCiti is a smart city, but our goal is a smart city that isn’t just underpinned by advance data and sensor technology only, but focus more on human development and good lifestyle of residents.
With Dciti blockchain implementation, we aim to build BitCiti to ensure environmental sustainability, fix social inequality and empower residents with needed resources to build a life they can be proud of.
...
Cross-laminated timber (CLT) is a large-scale, prefabricated, solid engineered wood panel. Lightweight yet very strong, with superior acoustic, fire, seismic, and thermal performance. CLT is also fast and easy to install, generating almost no waste on-site. CLT offers design flexibility and low environmental impacts. For these reasons, CLT or cross-laminated timber is proving to
21
________________
be a highly advantageous alternative to conventional materials like concrete, masonry, or steel, especially in multi-family and commercial construction.
